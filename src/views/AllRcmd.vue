<template>
	<div class="container-fluid p-0">
		...
		<nav class="navbar navbar-expand-lg border-bottom toolbar" :class="{'toolbar-up': headerHidden}">
			...
			<form class="toolbar-form">
				<transition name="stretch">
					<input class="form-control rounded-pill" :style="[isFocus ? '' : {width: searchText.length + 4 + 'rem'}]" type="text" placeholder="输入关键词搜索" v-if="showSearch" v-model.trim="searchText" @focus.passive="isFocus = true" @blur.passive="isFocus = false" v-focus>
				</transition>
		...
		<!-- 快捷按钮 -->
		<v-touch class="shortcut" id="to-top" @tap="onFold">
			<i class="fas fa-chevron-up"></i>
		</v-touch>
		<v-touch class="shortcut" @tap="onFold; onReload">
			<i class="fas fa-sync-alt" v-if="isReload" key="sync-icon"></i>
			<i class="fas fa-redo-alt" v-else key="redo-icon"></i>
		</v-touch>
		<v-touch class="shortcut" id="to-bottom" @tap="onFold">
