		...
		methods: {
			...
			loadData() {
				...
				.then(axios.spread(function (resFilms, resSeries) {
					...
					allRcmd.films = resFilms.data;
					console.log('getFilms:', allRcmd.films);
					allRcmd.series = resSeries.data;
					console.log('getSeries:', allRcmd.series);
					allRcmd.isReload = false;
					allRcmd.isSuccess = true;
					setTimeout(function () {
						allRcmd.showStatus = false;
					}, 1500);
				}))
				.catch(function (error) {
					console.error('catchError:', error);
					allRcmd.isReload = false;
					allRcmd.isError = true;
					allRcmd.showStatus = true;
				})
				.then(function () {
					console.log('finalData:', allRcmd.films, allRcmd.series);
				});
			},
			// 清空所有选中的标签
			clearTags() {
			...
			onSave() {
				var allRcmd = this;
				setTimeout(function () {
					allRcmd.showSaved = true;
					setTimeout(function () {
						allRcmd.showSaved = false;
					}, 2000);
				}, 1000);
			},
			// 评分七个级别显示不同颜色
